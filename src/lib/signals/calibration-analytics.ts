...
export function cumulativeReturn(
  ...
  if (overlapping) {
    ...
    for (const symbolRows of bySymbol.values()) {
      ...
      // Hoisted: the bar length is a property of the series, not of the row
      // being considered. Computing it inside the loop walked the whole symbol
      // once per sampled signal.
      const step = horizonBars * barMs(ordered);
