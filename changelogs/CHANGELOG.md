...
### Added (llm factor)
- Point-in-time inputs packet route (`GET /api/admin/llm-calls/inputs`, `src/app/api/admin/llm-calls/inputs/route.ts`), built by a pure, dependency-injected `buildInputsPacket` (`inputs/packet.ts`) so it is tested without Mongo or the network. For a symbol and interval (1h, 4h, 1d) it returns the last closed bar, its trailing closes, the latest global signal and history snapshot at or before that bar's close, and news published at or before it; nothing in the packet can postdate the close of the last closed bar. `inputsHash` is a sha256 over the packet body excluding `generatedAt`, so a repeated request for the same bar hashes identically and an `LlmCall` can prove what it read. A news feed failure yields an empty list rather than failing the request; 400 on a bad symbol or unsupported interval, 404 when no closed bar is stored, 401/403 via `authorizeLlmPanel`
