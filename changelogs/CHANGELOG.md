...
- The live signal engine scored the latest stored bar of the primary interval even when it was still open; only the HTF confirmation path filtered to closed bars. `computeSignalBatch` in `compute-engine.ts` now filters the primary interval's fetched candles to closed bars, same predicate as the HTF path, before computing indicators, score, `candleTimestamp`, and session, and skips the task with a log line if none remain. The legacy per-user path in `compute-signals/route.ts` applied the same filter, since it also read stored or freshly fetched candles directly
